...
integrators_split = [
    ("RK2", RK2Heuretic(test_metric, test_dtc_constant)),
    ("RK4", RK4Heuretic(test_metric, test_dtc_constant)),
    ("Split Hamiltonian (ω=$omega)", SplitHamiltonianHeuretic(test_metric, test_dtc_constant, omega))
]
...
for (idx, (integrator_name, integrator)) in enumerate(integrators_split)
    ax = Axis(fig[3, idx],
        xlabel="Timelike coordinate (t)",
        ylabel="ΔH",
...
Legend(fig[1:3, 4], [LineElement(color=color_map[k], linewidth=2) for k in state_keys],
    [string(k) for k in state_keys], "Starting States", framevisible=true)

Label(fig[0, :], "Hamiltonian Conservation: Adaptive vs Constant Timestep vs Split Hamiltonian", fontsize=24, font=:bold)
